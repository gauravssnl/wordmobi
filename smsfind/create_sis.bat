...
mkdir .\smsfind_sis\root\data\python\smsfind
copy  .\src\*.py   .\smsfind_sis\root\data\python\smsfind
move  .\smsfind_sis\root\data\python\smsfind\default.py   .\smsfind_sis

%PYTHON% .\ensymble.py py2sis --version="%VERSION%" --heapsize=4k,5M ^
          --appname="%APPNAME%" --caps="%CAPBLS%" ^
          --extrasdir=root smsfind_sis "%APPNAME%%VERSION%.sis"
